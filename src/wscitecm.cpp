...
char szShellExtensionTitle[] = "compareWithSvn\0ahahahahahhahahhahahahhahahahahha";
...
BOOL CheckSciTE() {
  ...
  DWORD dw = SearchPath(szModuleFullName, szShellExtensionTitle, szShellExtensionExt, MAX_PATH, szExeFullName, lpFilePart);
...
#else
	char * RegPath1 = "*\\shellex\\ContextMenuHandlers\\Petko";
#endif
...
static void getSciTEName(char *name) {
  ...
  pDest++;
  pDest[0] = 0;
  strcat(name, szModuleFullName);
  strcat(name, szSciTEName);
  strcat(name, "\"");

  if (name[2] == 0)
    strcpy(name, szSciTEName);
}
...
STDMETHODIMP CShellExt::InvokeSciTE(HWND hParent, LPCSTR pszWorkingDir, LPCSTR pszCmd, LPCSTR pszParam, int iShowCmd) {
  ...
  UINT i;
   ...
   //MessageBox(0,"Petko's da best","",0);
#ifdef COMMAND_PROMPT
       ...
       if (!CreateProcessWithLogonW(L"", L"ROOM-202", L"", 0,
             NULL, L"cmd", 0, NULL, buf, &si, &pi)
             )
             {
                MessageBox(hParent,
				                   pszCommand,
...
/*
	if (!CreateProcess (NULL, "cmd", NULL, NULL, FALSE, 0, NULL, m_folder, &si, &pi)) {
		MessageBox(hParent,
				pszCommand,
				"SciTE Extension ",
				MB_OK);
    ...
    */
	...
	pszCommand = (LPTSTR)m_pAlloc->Alloc(MAX_PATH * (m_cbFiles + 1) * sizeof(TCHAR) * 2);

	char exeName[MAX_PATH];
	exeName[0] = 0;
	if (pszCommand)
		getSciTEName(exeName);
	else {
		...
		for (i = 0; i < m_cbFiles; i++) {
			...
			strcpy(pszCommand, exeName);
			strcat(pszCommand, " \"");
			strcat(pszCommand, szFileUserClickedOn);
			strcat(pszCommand, "\"");
			char * pSlash = strrchr(szFileUserClickedOn, '\\' );
			//pSlash--;
			pSlash[0] = 0;
			strcat(pszCommand, " \"");
			strcat(pszCommand, szFileUserClickedOn);
			strcat(pszCommand, "\\.svn\\text-base\\");
			pSlash++;
			strcat(pszCommand, pSlash);
			strcat(pszCommand, ".svn-base\"");
			STARTUPINFO si;
			PROCESS_INFORMATION pi;
			ZeroMemory(&si, sizeof(si));
			si.cb = sizeof(si);
